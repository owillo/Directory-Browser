...
<input name="but" type="submit" value="Search" style="float:left;" /><BR style="clear:both;" /></form></div><?php  if(isset($_SESSION['search'])){  ?><div style="width:330px; float:left; font-size:14px; padding:10px;" >Search Results for: <span class="rStr">"<?php echo $_SESSION['search']; ?>"</span> returns <span class="rStr"><?php echo $addedstr; ?></span></div>
  ...
  <div id="listingheader"> 
	...
	if($dirok) {
	?>
      <div id=""><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?dir='.urlencode($dotdotdir);?>" id="headerx"><img src="<?php echo $includeurl; ?>dlf/dirup.png" alt="Folder" /><strong>..</strong> <em>&nbsp;</em>&nbsp;</a></div>
	<?php
	...
	for($i=0;$i<$arsize;$i++) {
		if(($class=='b d')){ $class='w d';}else if($class=='w d') {$class = 'b d';	};
	?>
	<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?dir='.urlencode(str_replace($startdir,'',$leadon).$dirs[$i]);?>" class="<?php echo $class;?>"><img src="<?php echo $includeurl; ?>dlf/folder.png" alt="<?php echo $dirs[$i];?>" /><span class="nm"><?php echo str_replace("/","",$dirs[$i]);?></span> <em>-</em> <span class="smaller"><?php echo date ("M d Y h:i:s A", filemtime($includeurl.$leadon.$dirs[$i]));?></span></a><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?dir='.urlencode(str_replace($startdir,'',$leadon).$dirs[$i]);?>" class="openfolder <?php echo $class;?>"  target="_blank" >&nbsp;</a></div>
	<?php
	...
	?>
	<div><a href="<?php echo $fileurl;?>" class="<?php echo $class;?>" <?php echo $thumb2;?>><img src="<?php echo $includeurl; ?>dlf/<?php echo $icon;?>" alt="<?php echo $files[$i];?>" /><span class="nm"><?php echo $filename;?></span> <em><?php echo formatBytes(filesize($fileurl),2);?></em> <span class="smaller"><?php echo date ("M d Y h:i:s A", filemtime($fileurl));?></span><?php echo $thumb;?></a>
